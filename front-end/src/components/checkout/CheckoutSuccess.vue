...
import {type Order, useCartStore} from '@/stores/useCart'
...
const processSuccessfulOrder = async (orderData: Order) => {
  try {
    ...
    if (result.success) {
      ...
      console.error('Cart clearing failed:', result.error)
    }
  } catch (err) {
  ...
  if (orderData.items) {
    for (const item of orderData.items) {
      if (item.product_id) {
        try {
          const response = await fetch(`${import.meta.env.VITE_USER_RELATED_SERVICE_URL}/user/history`, {
            method: 'POST',
            credentials: 'include',
            headers: {
          ...
          if (!response.ok) {
            console.error(`Failed to add item ${item.product_id} to history`)
          } else {
            console.log(`Added item ${item.product_id} to purchase history`)
          }
        } catch (err) {
          console.error(`Error adding item ${item.product_id} to history:`, err)
        }
      }
    }
  }
